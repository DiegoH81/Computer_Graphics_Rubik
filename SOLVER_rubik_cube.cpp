...
void RubikCube::Dump(const bool& is_color/* = false*/) {
    std::vector<char> line_buf(4 * (dim_ + 1) + 1);
...
void RubikCube::RotateCube(const ROTATE_CUBE_DIR& dir) {
    ...
    std::vector<char> tmp_face_buf(piece_num_);
    char* tmp_face = tmp_face_buf.data();
...
void RubikCube::RotateFace(const CUBE_FACE& rot_face, const ROTATE_DIR& dir, const bool& face_only/* = false*/) {
    ...
    std::vector<char> tmp_face_buf2(piece_num_);
    char* tmp_face = tmp_face_buf2.data();
...
std::string RubikCube::GetCubeString(const bool& is_color/* = false*/) {
    std::vector<char> tmp_faces_buf(std::strlen(faces_) + 1);
    char* tmp_faces = tmp_faces_buf.data();
